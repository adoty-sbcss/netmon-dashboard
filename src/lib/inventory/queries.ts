...
import { isCiscoIpPhoneName, looksLikeEndpoint, refineInfraType } from "@/lib/classify/device-hints";
...
export async function getInventoryForSchool(
  ...
  for (const sw of switches) {
    ...
    // Printers / UPSes / cameras answer SNMP and get crawled as switches — keep them
    // off the fabric inventory (they show with the host endpoints instead).
    if (looksLikeEndpoint(sw.systemDescription)) continue;
    const ip = sw.mgmtIp;
